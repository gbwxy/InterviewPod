...
export default function RealtimeSubtitle({
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-100 dark:bg-zinc-900 overflow-hidden">
      {/* Header */}
      <div className="px-5 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-zinc-200 dark:bg-zinc-800">
        <h4 className="text-sm font-semibold text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 tracking-wide">对话实录</h4>
        <div className="flex items-center gap-3">
          {isAiSpeaking && (
            <div className="flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 bg-zinc-200 dark:bg-zinc-800 rounded-full animate-pulse" />
              <span className="text-[10px] uppercase tracking-wider text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 font-semibold">AI 说</span>
            </div>
          )}
          {userText && (
            <div className="flex items-center gap-1.5">
      ...
      >
        <AnimatePresence initial={false}>
          ...
          {shouldShowActiveAi && (
            ...
            >
              <InterviewMessageBubble
                ...
                suffix={(
                  <motion.span
                    className="inline-block w-1.5 h-1.5 bg-zinc-200 dark:bg-zinc-800 ml-1.5 rounded-full"
          ...
          {messages.length === 0 && !userText && !aiText && (
            ...
            >
              <div className="w-12 h-12 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center mb-4 border border-zinc-200 dark:border-zinc-800">
