...
export default function AnalysisPanel({
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      ...
      default:
        return 'bg-zinc-200 dark:bg-zinc-800 border-zinc-300 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 dark:text-zinc-400';
    }
  };
  ...
  const getCategoryColor = (category: string) => {
    ...
    return colors[category] || 'bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 dark:text-zinc-400';
  };
  ...
  if (isProcessing) {
    ...
    return (
        <div className="bg-zinc-100 dark:bg-zinc-900 rounded-2xl p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 mb-2">
          ...
          <p className="text-sm text-zinc-500">页面将自动刷新显示分析结果</p>
      </div>
    );
  }
  ...
  if (analyzeStatus === 'FAILED' || !isAnalysisValid) {
    return (
        <div className="bg-zinc-100 dark:bg-zinc-900 rounded-2xl p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 mb-2">分析失败</h3>
          <p className="text-zinc-500 mb-4">AI 服务暂时不可用，请稍后重试</p>
        ...
        {onReanalyze && (
          <motion.button
            ...
            className="px-6 py-2.5 bg-black text-white dark:bg-white dark:text-black rounded-xl font-medium hover:bg-zinc-200 dark:bg-zinc-800 transition-colors disabled:opacity-50 flex items-center gap-2 mx-auto"
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <motion.button
              onClick={onExport}
              disabled={exporting}
              className="px-4 py-2 border border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900 rounded-lg text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 text-sm font-medium hover:bg-zinc-200 dark:bg-zinc-800 transition-all disabled:opacity-50 flex items-center gap-2"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
          ...
          <div
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="bg-zinc-100 dark:bg-zinc-900 rounded-xl p-5">
                <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 block mb-2">总分</span>
                <span className="text-4xl font-bold text-black dark:text-white">{analysis.overallScore || 0}</span>
      ...
      >
        ...
        <div className="space-y-6">
          ...
          {analysis.suggestions?.length === 0 && (
              <div className="text-center py-8 text-zinc-500">暂无改进建议</div>
          )}
        </div>
      </motion.div>
    </div>
      ...
      <div className="space-y-3">
        {suggestions.map((s: any, i: number) => (
          ...
          >
            ...
            <div className="mb-2">
              <p className="font-semibold text-black dark:text-white mb-1">{s.issue || '问题描述'}</p>
              <p className="text-sm leading-relaxed text-zinc-600 dark:text-zinc-400 dark:text-zinc-400">{s.recommendation || s}</p>
            </div>
          </motion.div>
        ))}
      </div>
