...
export default function HistoryList({onSelectResume}: HistoryListProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-black dark:text-white flex items-center gap-2.5">
            <FileStack className="w-5 h-5 text-zinc-600 dark:text-zinc-400 dark:text-zinc-400" />
            简历管理
          </h1>
          <p className="text-zinc-500 text-sm mt-1">管理您的简历，AI 智能分析与评分</p>
        </div>
      ...
      {/* 搜索栏 */}
      <div className="mb-5">
        <div className="flex items-center gap-3 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3.5 py-2.5 max-w-md focus-within:border-zinc-400 dark:border-zinc-600 transition-all">
          <svg className="w-4 h-4 text-zinc-600 dark:text-zinc-400" viewBox="0 0 24 24" fill="none">
          ...
          <input
            ...
            className="flex-1 outline-none text-zinc-500 dark:text-zinc-300 dark:text-zinc-300 placeholder:text-zinc-600 dark:text-zinc-400 bg-transparent text-sm"
          />
        </div>
      </div>
      ...
      {loading && (
        <div className="text-center py-20">
          <motion.div
            className="w-8 h-8 border-2 border-zinc-200 dark:border-zinc-800 border-t-zinc-500 rounded-full mx-auto mb-4"
            animate={{rotate: 360}}
      ...
      {!loading && filteredResumes.length === 0 && (
        <motion.div
          className="text-center py-20 bg-zinc-100 dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800"
          initial={{opacity: 0, scale: 0.95}}
          animate={{opacity: 1, scale: 1}}
        >
          <div className="text-5xl mb-5">📄</div>
          <h3 className="text-base font-medium text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 mb-1.5">暂无简历记录</h3>
          <p className="text-zinc-600 dark:text-zinc-400 text-sm">上传简历开始您的第一次 AI 面试分析</p>
        </motion.div>
      )}
        ...
        >
          <table className="w-full">
            ...
            <tr className="border-b border-zinc-200 dark:border-zinc-800">
              <th className="text-left px-5 py-3.5 text-[10px] font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest">简历名称</th>
              <th className="text-left px-5 py-3.5 text-[10px] font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest">上传日期</th>
              <th className="text-left px-5 py-3.5 text-[10px] font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest">分析状态</th>
              <th className="text-left px-5 py-3.5 text-[10px] font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest">AI 评分</th>
              <th className="text-left px-5 py-3.5 text-[10px] font-semibold text-zinc-600 dark:text-zinc-400 uppercase tracking-widest">面试状态</th>
            ...
            <AnimatePresence>
              {filteredResumes.map((resume, index) => (
                <motion.tr
                  ...
                  className="border-b border-zinc-200 dark:border-zinc-800 last:border-0 hover:bg-zinc-200 dark:bg-zinc-800 cursor-pointer transition-colors group"
                >
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div
                        className="w-8 h-8 bg-zinc-200 dark:bg-zinc-800 rounded-lg flex items-center justify-center text-zinc-500 group-hover:bg-zinc-300 dark:bg-zinc-700">
                      ...
                      <span className="font-medium text-sm text-zinc-900 dark:text-zinc-200">{resume.filename}</span>
                    </div>
                  </td>
                  <td className="px-5 py-4 text-xs text-zinc-500">{formatDateOnly(resume.uploadedAt)}</td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4">
                    {resume.analyzeStatus === 'COMPLETED' && resume.latestScore !== undefined ? (
                      <div className="flex items-center gap-2.5">
                        <div
                          className="w-16 h-1.5 bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden">
                          <motion.div
                        ...
                        <span className="font-semibold text-sm text-zinc-900 dark:text-zinc-200">{resume.latestScore}</span>
                      </div>
                    ) : isAnalyzing(resume.analyzeStatus) ? (
                      <span className="text-zinc-500 text-xs">生成中...</span>
                  ...
                  <td className="px-5 py-4">
                    {resume.interviewCount > 0 ? (
                      <span
                        className="inline-flex items-center gap-1 px-2.5 py-1 bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 rounded text-xs font-medium border border-zinc-300 dark:border-zinc-700">
                      ...
                      <span
                        className="inline-flex px-2.5 py-1 bg-zinc-100 dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 rounded text-xs border border-zinc-200 dark:border-zinc-800">待面试</span>
                    )}
                  </td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-1.5 text-zinc-600 dark:text-zinc-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        title="删除简历"
                      >
                      ...
                      <svg
                        className="w-4 h-4 text-zinc-500 dark:text-zinc-300 group-hover:text-zinc-600 dark:hover:text-zinc-400 dark:text-zinc-400 dark:text-zinc-400 group-hover:translate-x-0.5 transition-all"
