...
export default function VoiceInterviewEvaluationPage() {
    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          <div className="w-10 h-10 border-2 border-zinc-200 dark:border-zinc-800 border-t-zinc-500 rounded-full animate-spin mx-auto mb-4" />
          <p className="text-zinc-600 dark:text-zinc-400 dark:text-zinc-400">
            {evaluateStatus === 'PROCESSING' ? 'AI 正在分析面试表现...' : '正在生成评估报告...'}
          </p>
          <p className="text-zinc-500 text-sm mt-2">预计需要 10-30 秒</p>
        </div>
      </div>
    );
  }
  ...
  if (error && !evaluation) {
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          <p className="text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 text-lg mb-2">评估报告生成失败</p>
          <p className="text-zinc-500 text-sm mb-6">{error}</p>
          <div className="flex items-center gap-3 justify-center">
            <button
              onClick={handleRetry}
              className="px-6 py-2 bg-black text-white dark:bg-white dark:text-black rounded-lg hover:bg-zinc-200 dark:bg-zinc-800 flex items-center gap-2"
            >
            ...
            <button
              ...
              className="px-6 py-2 bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 rounded-lg hover:bg-zinc-200 dark:bg-zinc-800"
            >
              返回列表
            </button>
          </div>
    ...
    <div className="pb-10">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-black dark:text-white">面试评估报告</h1>
            <p className="text-sm text-zinc-500">语音会话 ID: {sessionId}</p>
          </div>
        </div>
