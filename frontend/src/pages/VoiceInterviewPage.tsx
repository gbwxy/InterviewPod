  ...
  if (!autoStartRef.current && !presetVoiceConfig && !resumeSessionId) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center p-6">
        <div className="bg-zinc-100 dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-8 text-center max-w-md w-full">
          <AlertCircle className="w-12 h-12 text-yellow-500 mx-auto mb-4" />
          <p className="text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 text-lg font-semibold mb-2">未检测到语音面试配置</p>
          <p className="text-zinc-500 text-sm mb-6">请从面试记录或"语音面试"入口开始</p>
          <button
            onClick={handleCloseModal}
            className="px-6 py-2 bg-black text-white dark:bg-white dark:text-black rounded-lg hover:bg-zinc-200 dark:bg-zinc-800 transition-colors"
          >
            返回重新开始
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-2 space-y-6">
            <div className="bg-zinc-100 dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-6">
              <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => navigate('/interviews')}
                    className="w-9 h-9 rounded-lg bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 hover:bg-zinc-200 dark:bg-zinc-800 transition-colors flex items-center justify-center"
                    title="返回面试记录"
                  >
                    <ArrowLeft className="w-4 h-4" />
                  </button>
                  <div>
                    <h2 className="text-lg font-semibold text-black dark:text-white">{templateName || effectiveSkillId}</h2>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs px-2 py-0.5 bg-zinc-200 dark:bg-zinc-800 dark:bg-zinc-800/40 text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 dark:text-zinc-400 rounded-full">
                ...
                <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 dark:text-zinc-400">
              ...
              <div className="flex flex-col items-center justify-center py-6">
                <motion.div
                  ...
                  className={`w-32 h-32 rounded-full border-4 flex items-center justify-center mb-6 transition-colors
                    ${isAiSpeaking
                      ? 'border-zinc-400 dark:border-zinc-600 bg-zinc-200 dark:bg-zinc-800 dark:bg-zinc-800/20'
                      : 'border-zinc-300 dark:border-zinc-700 bg-zinc-200 dark:bg-zinc-800/60'
                    }`}
                >
                  <Bot className={`w-14 h-14 ${isAiSpeaking ? 'text-zinc-600 dark:text-zinc-400 dark:text-zinc-400' : 'text-zinc-500'}`} />
                </motion.div>

                <div className="w-full max-w-2xl min-h-[130px] rounded-2xl bg-zinc-200 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-800 px-6 py-5 text-center flex items-center justify-center">
                  <AnimatePresence mode="wait">
                    {isAiSpeaking || aiText ? (
                      <motion.p
                        ...
                        exit={{ opacity: 0 }}
                        className="text-lg md:text-xl font-medium text-zinc-500 dark:text-zinc-300 dark:text-zinc-300 leading-relaxed"
                      >
                        {aiText || '思考中...'}
                      </motion.p>
                    ) : userText ? (
                      <motion.p
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0 }}
                        className="text-lg md:text-xl font-medium text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 dark:text-zinc-400 italic leading-relaxed"
                      >
                        {userText}
                      </motion.p>
                    ) : (
            ...
            <div className="bg-zinc-100 dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm p-5">
              <div className="flex items-center justify-center gap-6">
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-zinc-200 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 dark:text-zinc-400 hover:bg-zinc-200 dark:bg-zinc-800 transition-colors disabled:opacity-50"
                  title="暂停"
                >
                  暂停
                </button>
                ...
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl font-medium text-sm transition-all ${
                    canSubmit
                      ? 'bg-black text-white hover:bg-zinc-800 dark:bg-white dark:text-black dark:hover:bg-zinc-200 shadow-md '
          ...
          <div className="h-[520px] md:h-[560px] xl:h-[calc(100vh-240px)] xl:max-h-[760px] bg-zinc-100 dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
            <RealtimeSubtitle
              messages={messages}
              userText={userText}
              aiText={aiText}
